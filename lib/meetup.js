...
function refreshMemberships(groupId, callback) {
  ...
  function fetchMembers(error, response, body) {
    ...
    var members = parseMembers(data.results);
    ...
    if(data.meta.next && members.length > 0) {
      ...
      async.map(totalMembers, function(member, memberCb)
        $members.update({memberId: member.memberId, groupId: member.groupId}, {upsert: true, safe: true}, memberCb);
      , callback);
    }
  }
}
